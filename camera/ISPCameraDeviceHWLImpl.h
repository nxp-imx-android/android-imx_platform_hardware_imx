...
public:
    ISPCameraDeviceHwlImpl(uint32_t camera_id, /*std::vector<char*> devPath,*/ std::vector<std::shared_ptr<char*>> devPaths, std::vector<uint32_t> physicalIds,
      ...
      : CameraDeviceHwlImpl(camera_id, std::move(devPaths), std::move(physicalIds), cam_copy_hw, cam_csc_hw, hw_jpeg, use_cpu_encoder, cam_metadata, std::move(physical_devices), callback)
    {
    }
